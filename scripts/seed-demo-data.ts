...
async function seed(userId: string) {
  ...
  await supabase.from('automations').insert([
    ...
    { user_id: userId, name: 'Deal gone quiet or invoice overdue → Flag for follow-up', trigger_type: 'deals.needs_follow_up', action_type: 'deals.flag_follow_up', config: { description: 'Deals stuck 5+ days in Inbound/Negotiating, or with an unpaid invoice past its due date, already show on your Dashboard and in Deals right away, with no toggle needed. Turn this on for a daily scheduled check that logs what it found here too.' }, enabled: false, schedule: '0 13 * * *', schedule_label: 'Checked daily, 1pm UTC', created_at: daysAgo(6) },
    { user_id: userId, name: 'Monday digest: what\'s due this week', trigger_type: 'schedule.weekly_digest', action_type: 'deals.summarize_week', config: { description: 'Every Monday, a summary of deals and drafts due in the next 7 days.' }, enabled: false, schedule: '0 14 * * 1', schedule_label: 'Every Monday, 2pm UTC', created_at: daysAgo(6) },
  ])
