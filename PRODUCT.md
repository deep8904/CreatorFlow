...
- **Visual identity — superseded 2026-07-25**: the prior orange/black/white editorial system (built earlier this session) is being fully replaced per explicit user direction, not evolved. New visual authority is `DESIGN.md`, built from `nexus-architecture-DESIGN.md` (dark #000/#232323 ground, #22C55E accent, Inter display + JetBrains Mono body/label type, 8px spacing/radius system, pill controls, masked/staggered motion, ambient WebGL/canvas layers) as primary reference, with the Skymetrics dashboard screenshots as secondary reference for in-app data-density patterns (stat tiles, sidebar, charts, light/dark pairing). The logo mark, favicon, and CreatorFlow name stay; their rendering (color, treatment) follows the new system.
